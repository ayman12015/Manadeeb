          ...
          <div class="col-sm-3">
            <section class="panel">
              <div class="panel-body">     
                         <h4 class="gen-case">
                    {$CustomersManagement_Users_List}                   
                  </h4>
                <ul class="nav nav-pills nav-stacked mail-nav">
   
{foreach from=$Users item=$user} 
{if $selected eq $user.id}
<li class="active"><a href="?selected={$user.id}"><i class="fa fa-male"></i> {$user.username}  <span class="label label-theme pull-right inbox-notification">{$user.CustomerCount}</span></a></li>
{else}
<li class=""><a href="?selected={$user.id}"> <i class="fa fa-male"></i> {$user.username}   <span class="label label-theme pull-right inbox-notification"> {$user.CustomerCount}</span></a></li> 
{/if}
{/foreach}
                ...
                <div class="mail-option">
                  ...
                  <div class="btn-group hidden-phone">
                    ...
                    <ul class="dropdown-menu">
                      ...
                      {foreach from=$Users item=$user} 
						{if $selected neq $user.id}
                          <li onclick="GetAction('{$selected}','{$user.id}','usr')"><a href="#"><i class="fa fa-user"></i> {$user.username}</a></li>
                        {/if}
					  {/foreach}
                  ...
                  <table class="table table-inbox table-hover">
                    ...
                    {foreach from=$CUSTOMERS item=$CUSTOMER} 
                      <tr class="">
                        ...
                        <td class="view-message  text-right">{$rptUserCustomers_REG_Date}: {$CUSTOMER.entered_date}</td>
                      </tr>
                    {/foreach}
                    </form>
                    </tbody>
